...
// // import { Calendar, Clock, User, AlertCircle, ChevronDown, ChevronRight, ArrowLeft, Inbox, Send, CheckCircle, XCircle, Plus, Search, Bell, Filter, Link2 } from 'lucide-react';
...
// //         const incomerequests = await axios.get(`http://localhost:5181/api/User/Income/${user.id}`, config);
...
// //       try {
// //         const config = await getAuthHeaders();
// //         const response = await axios.get(`http://localhost:5181/api/User/${user.id}`, config);
...
// //     try {
// //       const config = await getAuthHeaders();
// //       console.log('Rejecting dependency for task ID:', request.dependencyTaskId);
// //       await axios.post(`http://localhost:5181/api/Dependency/RejectDependency/${request.dependencyTaskId}`, {}, config);
...
// //       alert('Failed to reject request: ' + (error.response?.data?.message || error.message || 'Please try again.'));
// //     }
// //   };

// //   const handleCreateTask = (request) => {
...
// //       try {
// //         const config = await getAuthHeaders();
// //         await axios.put(`http://localhost:5181/api/User/${selectedRequest.id}`, { status: 'approved' }, config);
// //         const incomerequests = await axios.get(`http://localhost:5181/api/User/Income/${user.id}`, config);
// //         const sortedRequests = incomerequests.data.sort((a, b) => {
// //           const statusOrder = { pending: 1, approved: 2, rejected: 3, accepted: 4 };
// //           const statusDiff = statusOrder[a.status?.toLowerCase()] - statusOrder[b.status?.toLowerCase()];
...
// //       await axios.patch(`http://localhost:5181/api/Task/link/${task}/${request.dependencyTaskId}`, config);
...
// //                 <span>{isIncoming ? `From: ${cleanName(decryptString(request.name, aesKey, aesIV))}` : `To: ${cleanName(decryptString(request.name, aesKey, aesIV))}`}</span>
// //               </div>
// //             </div>
...
// //               <div>
// //                 <h4 className="font-medium text-gray-900 mb-2">Requested Task:</h4>
// //                 <p className="text-gray-700 text-sm leading-relaxed">{request.requestedTask}</p>
// //               </div>
// //               <div>
...
// //               {!isIncoming && request.status?.toLowerCase() === 'accepted' && (
// //                 <div className="flex gap-2 pt-2">
...
// //             <p className="text-gray-600">Manage incoming and outgoing dependency requests</p>
// //           </div>
...
// // import { fetchEncryptionKeys } from '../../services/apiClient';
// // import { decryptString } from '../../services/decrypt';
// // import { Calendar, Clock, User, AlertCircle, ChevronDown, ChevronRight, ArrowLeft, Inbox, Send, CheckCircle, Search, Link2, Eye, XCircle } from 'lucide-react';
// // import Button from '../../common-components/Button';
...
// //   // const handleLinkToExistingTask = async (request) => {
// //   //   const taskId = prompt('Enter the Task ID to link to this request:');
// //   //   if (!taskId) return;
// //   //   try {
// //   //     const config = await getAuthHeaders();
// //   //     await axios.put(`http://localhost:5181/api/User/${request.id}`, { status: 'approved' }, config);
// //   //     alert(`Successfully linked to task #${taskId}`);
// //   //     const incomerequests = await axios.get(`http://localhost:5181/api/User/Income/${user.id}`, config);
// //   //     const sortedRequests = incomerequests.data.sort((a, b) => {
// //   //       const statusOrder = { pending: 1, approved: 2, rejected: 3, accepted: 4 };
...
// //     console.log('RequestCard data:', request); // Debug log for request data
...
// //                   </button>
// //                   {/* <button
// //                     onClick={() => handleLinkToExistingTask(request)}
// //                     className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium"
// //                   >
...
//           </button>
//           <div className="p-4">
//             <h3 className="text-lg font-semibold text-gray-900 mb-4">{request.taskName || 'Untitled Request'}</h3>
