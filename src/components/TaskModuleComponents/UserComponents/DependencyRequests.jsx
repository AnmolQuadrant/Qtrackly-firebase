...
import { fetchEncryptionKeys } from '../../services/apiClient';
import { decryptString } from '../../services/decrypt';
import { Calendar, Clock, User, AlertCircle, ArrowLeft, Inbox, Send, CheckCircle, Search, Link2, XCircle, ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react';
import Button from '../../common-components/Button';
import TaskLinkDropdown from '../../TaskModuleComponents/UserComponents/TaskLinkDropdown';
...
const DependencyRequests = () => {
  ...
  const fetchIncomingRequests = useCallback(async (showRefreshIndicator = false) => {
    if (!user?.id || !isAuthenticated) return;
    
    try {
      if (showRefreshIndicator) setRefreshing(true);
      const config = await getAuthHeaders();
      const response = await axios.get(`https://qtrackly-awd6egbkg8dbaeex.centralindia-01.azurewebsites.net/api/User/Income/${user.id}`, config);
  ...
  }, [user?.id, isAuthenticated, acquireToken]);

  // Fetch outgoing requests
  const fetchOutgoingRequests = useCallback(async (showRefreshIndicator = false) => {
    if (!user?.id || !isAuthenticated) return;
  ...
  const fetchAllData = useCallback(async (showRefreshIndicator = false) => {
    ...
    try {
      await Promise.all([
        fetchIncomingRequests(showRefreshIndicator),
        fetchOutgoingRequests(showRefreshIndicator)
      ]);
  ...
  }, [fetchIncomingRequests, fetchOutgoingRequests, user?.id, isAuthenticated]);

  // Initial data fetch
  useEffect(() => {
    if (isAuthenticated && user?.id && aesKey && aesIV) {
      fetchAllData();
    }
  }, [isAuthenticated, user?.id, aesKey, aesIV, fetchAllData]);
  ...
  const handleTaskAdded = async () => {
    if (selectedRequest) {
      try {
        ...
        console.error('Error updating request status:', error);
        alert('Request has been Accepted');
        // Refresh even on error to ensure data consistency
        await fetchIncomingRequests();
      }
    }
    handleAddTaskModalClose();
  ...
  const handleFormSave = async (taskId, stepId, newRequest) => {
    setOutgoingRequests(prev => [{
      id: Date.now(),
      taskName: newRequest.notes,
      requestedTask: newRequest.notes,
    ...
    // Refresh outgoing requests after some time to get the actual data from server
    setTimeout(() => {
