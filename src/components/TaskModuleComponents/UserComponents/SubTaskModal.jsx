...
function SubtaskModal({ taskId, editingStepId, stepForm, setStepForm, onClose, onSave }) {
  ...
  // Initialize stepForm for new subtasks with current date
  useEffect(() => {
    if (!editingStepId) {
      const currentDate = new Date('2025-08-19').toISOString().split('T')[0]; // Format as YYYY-MM-DD
      setStepForm({
        name: '',
        description: '',
  ...
  const handleSave = useCallback(async () => {
    ...
    if (stepForm.estimatedHours && parseFloat(stepForm.estimatedHours) <= 0) {
      setError('Estimated Hours must be greater than 0');
      setIsSaving(false);
      return;
    }
    ...
    try {
      const formattedSubtask = {
        ...
        priority: validatedPriority,
        taskItemId: parsedTaskId,
        estimatedHours: parseFloat(stepForm.estimatedHours) || 1,
        workedHours: parseFloat(stepForm.todayEffort) || 0,
        ...(editingStepId
          ? { datetime: formatDate(stepForm.date) }
          : { startDate: formatDate(stepForm.startDate), dueDate: formatDate(stepForm.dueDate) })
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-[458px] mx-4 max-h-[80vh] flex flex-col">
