...
function Pagination({ paginationData, onPageChange }) {
    ...
    const getPageNumbers = () => {
        ...
        if (totalPages <= maxVisiblePages) {
            ...
            // Calculate start and end pages for a three-page sliding window
            let startPage = Math.max(1, currentPage - Math.floor(maxVisiblePages / 2));
            let endPage = Math.min(totalPages, startPage + maxVisiblePages - 1);

            // Adjust startPage to ensure a full window when near the end
            if (endPage - startPage + 1 < maxVisiblePages) {
                startPage = Math.max(1, totalPages - maxVisiblePages + 1);
                endPage = totalPages;
            }

            // Generate page numbers within bounds
            for (let i = startPage; i <= endPage; i++) {
                pages.push(i);
            }
        }

        console.log('Pagination page window:', { totalPages, currentPage, pages });
        return pages;
    };
